...
class DirectoryDataGenerator(keras.utils.Sequence):
    def __init__(self, base_directories, augmentor=False, preprocessors=None, batch_size=16, target_sizes=(224, 224),
        ...
        for base_directory in base_directories:
            class_names = [x for x in listdir(base_directory) if
                           isdir(join(base_directory, x))]  # only folders are added
            ...
            for i, c in enumerate(class_names):
                ...
                if isdir(class_dir):
                    ...
                    for f in listdir(class_dir):
                        ...
                        if isfile(
                                file_dir):  # TODO: replace this with a white list (.png, .jpg, etc.), so we only consider valid image files and not all files!
